...
class numinwordsENTest(TestCase):
    def test_ordinal_special_joins(self):
        self.assertEqual(
            numinwords(5, ordinal=True, lang='fr_CH'), "cinquième"
        )
        self.assertEqual(
            numinwords(6, ordinal=True, lang='fr_CH'), "sixième"
        )
        self.assertEqual(
            numinwords(35, ordinal=True, lang='fr_CH'), "trente-cinquième"
        )
        self.assertEqual(numinwords(9, ordinal=True, lang='fr_CH'), "neuvième")
        self.assertEqual(
            numinwords(49, ordinal=True, lang='fr_CH'), "quarante-neuvième"
        )
        self.assertEqual(numinwords(71, lang='fr_CH'), "septante et un")
        self.assertEqual(numinwords(81, lang='fr_CH'), "huitante et un")
        self.assertEqual(numinwords(80, lang='fr_CH'), "huitante")
        self.assertEqual(numinwords(880, lang='fr_CH'), "huit cents huitante")
        self.assertEqual(
            numinwords(91, ordinal=True, lang='fr_CH'), "nonante et unième"
        )
        self.assertEqual(numinwords(53, lang='fr_CH'), "cinquante-trois")

    def test_number(self):
        for test in TEST_CASES_CARDINAL:
            self.assertEqual(numinwords(test[0], lang='fr_CH'), test[1])

    def test_ordinal(self):
        for test in TEST_CASES_ORDINAL:
            self.assertEqual(
                numinwords(test[0], lang='fr_CH', ordinal=True),
                test[1]
            )

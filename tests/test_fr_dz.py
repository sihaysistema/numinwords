...
class numinwordsPLTest(TestCase):
    def test_currency(self):
        self.assertEqual(
            numinwords(1234.12, lang='fr_DZ', to='currency'),
            "mille deux cent trente-quatre dinards et douze centimes"
        )
        self.assertEqual(
            numinwords(45689.89, lang='fr_DZ', to='currency'),
    ...
    def test_number(self):
        for test in test_fr.TEST_CASES_CARDINAL:
            self.assertEqual(numinwords(test[0], lang='fr_DZ'), test[1])

    def test_ordinal(self):
        for test in test_fr.TEST_CASES_ORDINAL:
            self.assertEqual(
                numinwords(test[0], lang='fr_DZ', ordinal=True),
                test[1]
            )
    ...
    def test_ordinal_num(self):
        for test in test_fr.TEST_CASES_ORDINAL_NUM:
            self.assertEqual(
                numinwords(test[0], lang='fr_DZ', to='ordinal_num'),
                test[1]
            )

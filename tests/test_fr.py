...
class numinwordsENTest(TestCase):
    def test_ordinal_special_joins(self):
        # ref https://github.com/savoirfairelinux/numinwords/issues/18
        self.assertEqual(
            numinwords(5, ordinal=True, lang='fr'), "cinquième"
        )
        self.assertEqual(
            numinwords(35, ordinal=True, lang='fr'), "trente-cinquième"
        )
        self.assertEqual(
            numinwords(9, ordinal=True, lang='fr'), "neuvième"
        )
        self.assertEqual(
            numinwords(49, ordinal=True, lang='fr'), "quarante-neuvième"
        )

    def test_number(self):
        for test in TEST_CASES_CARDINAL:
            self.assertEqual(numinwords(test[0], lang='fr'), test[1])

    def test_ordinal(self):
        for test in TEST_CASES_ORDINAL:
            self.assertEqual(
                numinwords(test[0], lang='fr', ordinal=True),
                test[1]
            )
    ...
    def test_ordinal_num(self):
        for test in TEST_CASES_ORDINAL_NUM:
            self.assertEqual(
                numinwords(test[0], lang='fr', to='ordinal_num'),
                test[1]
            )
    ...
    def test_currency_eur(self):
        for test in TEST_CASES_TO_CURRENCY_EUR:
            self.assertEqual(
                numinwords(test[0], lang='fr', to='currency', currency='EUR'),
                test[1]
            )
    ...
    def test_currency_frf(self):
        for test in TEST_CASES_TO_CURRENCY_FRF:
            self.assertEqual(
                numinwords(test[0], lang='fr', to='currency', currency='FRF'),
                test[1]
            )
    ...
    def test_currency_usd(self):
        for test in TEST_CASES_TO_CURRENCY_USD:
            self.assertEqual(
                numinwords(test[0], lang='fr', to='currency', currency='USD'),
                test[1]
            )
